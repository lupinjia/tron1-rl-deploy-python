...
class PointfootEEController:
    def __init__(self, model_dir, robot, robot_type, rl_type, 
                 start_controller, config_filename="params.yaml",
                 use_usb_joy=False):
        # Initialize robot and type information
        self.robot = robot
        self.robot_type = robot_type
        self.rl_type = rl_type
        self.use_usb_joy = use_usb_joy

        # Load configuration and model file paths based on robot type
        self.config_file = f'{model_dir}/{self.robot_type}/{config_filename}'
        
        # Load configuration settings from the YAML file
        self.load_config(self.config_file)
        
        self.model_policy = f'{model_dir}/{self.robot_type}/policy/{self.rl_type}/{self.policy_name}'
        ...
        if self.use_usb_joy:
            print("Use USB joystick for command input")
            self.joystick = Joystick()
        else:
            print("Use robot SensorJoy for command input")
    ...
    def initialize_onnx_models(self):
        ...
        print(f"Loaded ONNX model from {self.model_policy}")
        print(f"Policy input names: {self.policy_input_names}, shapes: {self.policy_input_shapes}")
        print(f"Policy output names: {self.policy_output_names}, shapes: {self.policy_output_shapes}")
    ...
    def load_config(self, config_file):
        ...
        self.sit_joint_angle = config['PointfootCfg']['sit_mode']['sit_joint_angle']
        self.sit_duration = config['PointfootCfg']['sit_mode']['sit_duration']
        ...
        self.policy_name = config['PointfootCfg']['policy_name']
        self.gait_period = config['PointfootCfg']['gait']['gait_period']
        self.theta = np.zeros(2)
        self.theta[0] = config['PointfootCfg']['gait']['theta_left']
        self.theta[1] = config['PointfootCfg']['gait']['theta_right']

        # Initialize variables for actions, observations, and commands
        self.obs_history_deque = deque(maxlen=self.obs_history_length)
        ...
        self.sit_percent = 0  # percentage of time the robot has spent in sit mode
        self.policy_session = None  # ONNX model session for policy inference
        self.joint_num = len(self.joint_names)  # number of joints
        self.gait_time = 0.0  # time variable for gait generation
        self.phi = 0.0  # phase variable for gait generation
        self.clock_input = np.zeros(4, dtype=np.float32)  # clock input for observation
    ...
    # Handle the damping mode where the robot joints are relaxed
    def handle_damping_mode(self):
        for j in range(len(self.joint_names)):
            self.set_joint_command(j, 0, 0, 0, 0, 4.0)

    # Handle the sit mode for smoothly transitioning the robot into siting
    def handle_sit_mode(self):
        for j in range(len(self.joint_names)):
            # Interpolate between sit initial and sit joint angles during sit mode
            pos_des = self.sit_initial_joint_angles[j] * (1 - self.sit_percent) + self.sit_joint_angle[self.joint_names[j]] * self.sit_percent
            self.set_joint_command(j, pos_des, 0, 0, self.control_cfg['stiffness'], self.control_cfg['damping'])
        # Increment the sit percentage over time
        self.sit_percent = self.sit_percent + 1 / (self.sit_duration * self.loop_frequency) if self.sit_percent < 1.0 else 1.0
    ...
    def calc_periodic_reward_obs(self):
        # Update gait time and phase variable
        dt = 1.0 / (self.loop_frequency / self.control_cfg['decimation'])
        self.gait_time += dt
    ...
    def compute_observation(self):
        ...
        self.commands[0] = -self.joystick.ly * self.user_cmd_cfg['lin_vel_x']
        self.commands[1] = -self.joystick.lx * self.user_cmd_cfg['lin_vel_y']
        self.commands[2] = -self.joystick.rx * self.user_cmd_cfg['ang_vel_yaw']
        self.scaled_commands[0] = self.commands[0] * self.obs_scales["lin_vel"]
        self.scaled_commands[1] = self.commands[1] * self.obs_scales["lin_vel"]
        self.scaled_commands[2] = self.commands[2] * self.obs_scales["ang_vel"]
        ...
        obs = np.concatenate([
            self.scaled_commands,  # Scaled velocity commands
            ...
            self.clock_input  # Clock input for gait phase information
        ])
        
        # Update the observation history deque
        self.obs_history_deque.append(obs)
    ...
    def compute_actions(self):
        ...
        input_tensor = self.obs_history[np.newaxis, :].astype(np.float32) # Add batch dimension
    ...
    def update_state_machine(self):
        """
        Updates the state machine based on the current mode and conditions.
        """
        if self.joystick.button[KeyMap.L1].pressed and self.joystick.button[KeyMap.Y].on_press:
            print(f"L1 + Y: DAMPING mode...")
            self.mode = "DAMPING"
            ...
            # Refresh the observation history deque
            self.obs_history_deque.clear()
            for _ in range(self.obs_history_length):
                self.obs_history_deque.append(np.zeros(self.observations_size))
            self.mode = "WALK"
    ...
    def update(self):
        ...
        # Read joystick inputs for linear and angular velocities
        pygame.event.pump()  # Process pygame events to get joystick input
        self.joystick.update()
        # Update State Machine based on current mode
        self.update_state_machine()
        
        if self.mode == "DAMPING":
            self.handle_damping_mode()
        elif self.mode == "SIT":
            self.handle_sit_mode()
        elif self.mode == "WALK":
        ...
        if self.loop_count % self.loop_frequency == 0:
